...
//       todo if (labelset_validate(labelset, labelset_len) != 0) return -1;
        ...
        if (extra == null && extra.length != 0) return false;
        if (extra != null && extra.length == 0) return false;
        if (extra != null && labelset_is_empty(labelset)) return false;
        ...
        prefix_len += ((BLOCKLEN - (prefix_len % BLOCKLEN)) % BLOCKLEN);
        prefix_len += labelset.length + POINTLEN + extra.length;
        if (prefix_len > M_start) return false;
...
//            prefix_len = 3 * POINTLEN + 2 * labelset.length + extra.length;
    ...
    public static int generalized_solve_commitment(byte[] R_bytes_out, ge_p3 K_point_out,
        ...
        if (B_point == null) {
            ...
            // s * Bv
            ge_scalarmult_base.ge_scalarmult_base(sB, s_scalar);

            // h * -K
            ge_scalarmult_base.ge_scalarmult_base(hK, h_scalar);
...
//            return -1;
    ...
    public static boolean generalized_veddsa_25519_sign(
        ...
        if (eddsa_25519_pubkey_bytes == null) return false;
        if (eddsa_25519_privkey_scalar == null) return false;
        if (msg == null) return false;
        if (customization_label == null && customization_label_len != 0) return false;
        if (customization_label_len > LABELMAXLEN) return false;
        if (msg_len > MSGMAXLEN) return false;
...
//        memcpy(extra + POINTLEN, Kv_bytes, POINTLEN);
        ...
        if (!generalized_commit(sha512provider, R_bytes, r_scalar,
                ...
                random, M_buf, MSTART, msg_len)) return false;

        //  Rv = r * Bv
        ge_scalarmult_base.ge_scalarmult_base(Rv_point, r_scalar);
...
//        memcpy(extra + 2*POINTLEN, Rv_bytes, POINTLEN);
        ...
        if (generalized_challenge(sha512provider, h_scalar,
                labelset, extra, R_bytes, eddsa_25519_pubkey_bytes) != 0) return false;

        //  s = prove(r, k, h)
        if (generalized_prove(s_scalar, r_scalar, eddsa_25519_privkey_scalar, h_scalar) != 0) return false;
...
//        bufptr = buffer_add(bufptr, labelset + labelset_maxlen, customization_label, customization_label_len);
    ...
    public static byte[] labelset_add(byte[] labelset, String label)
    {
        ...
        if (bb.position() - labelset.length >= LABELSETMAXLEN)
            throw new IllegalStateException();
        if (bb.position() - labelset.length != labelset.length + 1 + label.length())
            throw new IllegalStateException();
...
//        if (!sc_isreduced(s_scalar)) return -1;
        ...
        //  Bv = hash(hash(labelset1 || K) || M)
        labelset_add(labelset, "1");
        if (!generalized_calculate_Bv(sha512provider, Bv_point, labelset, eddsa_25519_pubkey_bytes, M_buf, MSTART, msg_len)) return false;
        ge_p3_tobytes.ge_p3_tobytes(Bv_bytes, Bv_point);

        //  R = solve_commitment(B, s, K, h)
        if (generalized_solve_commitment(R_calc_bytes, K_point, null,
                s_scalar, eddsa_25519_pubkey_bytes, h_scalar) != 0) return false;

        //  Rv = solve_commitment(Bv, s, Kv, h)
        if (generalized_solve_commitment(Rv_calc_bytes, Kv_point, Bv_point,
                s_scalar, Kv_bytes, h_scalar) != 0) return false;
